...
function AssignRoles() {
    ...
    const styles = {
        container: {
          ...
          backgroundImage: `url(${process.env.PUBLIC_URL + '/orange.jpg'})`, // Path to your image
          backgroundSize: 'cover', // Scales the image to cover the entire screen
    ...
    const loadBlockchaindata = async () => {
        ...
        if (networkData) {
            ...
            const rms = {};
            for (i = 1; i <= rmsCtr; i++) {
                rms[i] = await supplychain.methods.rawMaterialSuppliers(i).call();
            }
            setRMS(rms);
            const manCtr = await supplychain.methods.manCtr().call();
            const man = {};
            for (i = 1; i <= manCtr; i++) {
                man[i] = await supplychain.methods.manufacturers(i).call();
            }
            setMAN(man);
            const disCtr = await supplychain.methods.disCtr().call();
            const dis = {};
            for (i = 1; i <= disCtr; i++) {
                dis[i] = await supplychain.methods.distributors(i).call();
            }
            setDIS(dis);
            const retCtr = await supplychain.methods.retCtr().call();
            const ret = {};
            for (i = 1; i <= retCtr; i++) {
                ret[i] = await supplychain.methods.retailers(i).call();
            }
            setRET(ret);
            setloader(false);
        }
    ...
    return (
        <div style={styles.container}>
            ...
            <span style={{background:'red', color:'white'}} onClick={redirect_to_home} className="btn btn-outline-danger btn-sm">HOME</span>
            ...
            <form onSubmit={handlerSubmitRMS}>
                ...
                <button style={styles.btn} className="btn btn-outline-success btn-sm" onSubmit={handlerSubmitRMS}>Register</button>
            </form>
            ...
            <form onSubmit={handlerSubmitMAN}>
                ...
                <input className="
                -control-sm" type="text" onChange={handlerChangePlaceMAN} placeholder="Based In" required />
                <button style={styles.btn} className="btn btn-outline-success btn-sm" onSubmit={handlerSubmitMAN}>Register</button>
            </form>
            <table className="table table-sm">
                <thead>
                    <tr>
            ...
            <form onSubmit={handlerSubmitDIS}>
                ...
                <button style={styles.btn} className="btn btn-outline-success btn-sm" onSubmit={handlerSubmitDIS}>Register</button>
            </form>
            <table className="table table-sm">
                <thead>
                    <tr>
            ...
            <form onSubmit={handlerSubmitRET}>
                ...
                <button  style={styles.btn}  className="btn btn-outline-success btn-sm" onSubmit={handlerSubmitRET}>Register</button>
            </form>
            <table className="table table-sm">
                <thead>
                    <tr>
